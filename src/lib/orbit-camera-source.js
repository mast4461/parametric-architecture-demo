...
// From https://github.com/playcanvas/engine
...
export function createOrbitCameraScripts() {
  const noop = () => {};
  var OrbitCamera = pc.createScript(scriptNames.OrbitCamera);
  ...
  OrbitCamera.attributes.add('scaleFactor', {
      type: 'number',
      default: 1,
      title: 'Scale factor',
      description: 'The number by which the geometry is scaled, if it is scaled. Used to scale camera settings proportionally.'
  });
  ...
  OrbitCamera.attributes.add('startAutoRender', {
      type: 'function',
      default: noop,
  });
  OrbitCamera.attributes.add('stopAutoRender', {
      type: 'function',
      default: noop,
  });
  OrbitCamera.attributes.add('render', {
      type: 'function',
      default: noop,
  });
  ...
  ////////////////////////////////////////////////////////////////////////////////
  var OrbitCameraInputMouse = pc.createScript(scriptNames.OrbitCameraInputMouse);
  ...
  OrbitCameraInputMouse.prototype.initialize = function () {
      ...
      if (this.orbitCamera) {
          ...
          Object.entries(listeners).forEach(([eventName, listener]) =>
            pc.app.graphicsDevice.canvas.addEventListener(eventName, listener)
          );

          // Listen to when the mouse travels out of the window
          pc.app.graphicsDevice.canvas.addEventListener('mouseout', onMouseOut, false);

          // Remove the listeners so if this entity is destroyed
          this.on('destroy', function () {
              Object.entries(listeners).forEach(([eventName, listener]) => 
                pc.app.graphicsDevice.canvas.removeEventListener(eventName, listener)
              );

              pc.app.graphicsDevice.canvas.removeEventListener('mouseout', onMouseOut, false);
          });
      }
  ...
  OrbitCameraInputMouse.prototype.pan = function (event) {
      ...
      camera.screenToWorld(event.clientX, event.clientY, distance, fromWorldPoint);
      camera.screenToWorld(this.lastPoint.x, this.lastPoint.y, distance, toWorldPoint);

      worldDiff.sub2(toWorldPoint, fromWorldPoint).scale(1 / this.orbitCamera.scaleFactor);
  ...
  OrbitCameraInputMouse.prototype.onMouseDown = function (event) {
      this.orbitCamera.startAutoRender();
      switch (event.button) {
  ...
  OrbitCameraInputMouse.prototype.onMouseUp = function (event) {
      this.orbitCamera.stopAutoRender();
      switch (event.button) {
          case pc.MOUSEBUTTON_LEFT:
              this.lookButtonDown = false;
              break;
  ...
  OrbitCameraInputMouse.prototype.onMouseMove = function (event) {
      if (this.lookButtonDown) {
          this.orbitCamera.pitch -= event.movementY * this.orbitSensitivity;
          this.orbitCamera.yaw -= event.movementX * this.orbitSensitivity;
  ...
  OrbitCameraInputMouse.prototype.onMouseWheel = function (event) {
    this.orbitCamera.distance -= event.wheelDelta / 120 * this.distanceSensitivity * (this.orbitCamera.distance * 0.1);
    event.preventDefault();
    this.orbitCamera.render();
  };
  ...
  ////////////////////////////////////////////////////////////////////////////////
  var OrbitCameraInputTouch = pc.createScript(scriptNames.OrbitCameraInputTouch);
